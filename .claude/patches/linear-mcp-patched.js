...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
        ...
        {
            name: "list_states",
            description: "List workflow states for a team. Returns state IDs needed for update_issue status changes.",
            inputSchema: {
                type: "object",
                properties: {
                    teamId: {
    ...
    try {
        switch (request.params.name) {
            ...
            case "update_issue": {
                ...
                // Resolve status name to stateId UUID if needed
                let resolvedStateId = args.status;
                if (args.status && !/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(args.status)) {
                    const team = await issue.team;
                    if (team) {
                        const states = await team.states();
                        const match = states.nodes.find(s => s.name.toLowerCase() === args.status.toLowerCase());
                        if (!match) {
            ...
            case "list_states": {
                ...
                const states = await team.states();
                const formattedStates = states.nodes.map(s => ({
                    id: s.id,
                    name: s.name,
                    type: s.type,
                    position: s.position,
